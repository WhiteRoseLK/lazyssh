...
func moveFormFocus(form *tview.Form, direction int) bool {
	...
	form.SetFocus(next)
	form.GetFormItem(itemIndex).Blur()
	if next < form.GetFormItemCount() {
		form.GetFormItem(next).Focus(func(p tview.Primitive) {})
	} else if bIdx := next - form.GetFormItemCount(); bIdx < form.GetButtonCount() {
		form.GetButton(bIdx).Focus(func(p tview.Primitive) {})
	}
	return true
}
...
func (sf *ServerForm) addDropDownWithHelp(form *tview.Form, label, fieldName string, options []string, initialOption int) {
	...
	// Keep option navigation cyclic while the dropdown list is open.
	dropdown.SetInputCapture(func(event *tcell.EventKey) *tcell.EventKey {
		...
		var direction int
		//nolint:exhaustive // We only handle arrow navigation
		switch event.Key() {
		case tcell.KeyDown:
			direction = 1
		case tcell.KeyUp:
...
func (sf *ServerForm) hasUnsavedChanges() bool {
	...
	orig := sf.original
	if orig == nil && sf.mode == ServerFormAdd {
		orig = sf.initialData
	}
	...
	if orig != nil {
		...
		if reflect.DeepEqual(currentServer, *orig) {
			return false
		}

		// If DeepEqual says they're different, use our custom comparison
		// that handles nil vs empty slice and other normalization
		return sf.serversDiffer(currentServer, *orig)
	}

	// For new servers without original or initialData, check if any required fields are non-empty
